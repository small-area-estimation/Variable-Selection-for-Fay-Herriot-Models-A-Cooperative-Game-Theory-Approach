...
full.model <- eblupFH(yd ~ 0 + X1 + X2 + X3 , vardir = sigma2d, data = data_example)
full.model
...
for(i in seq_along(comb_var)){
  ...
  model.i <- eblupFH(formula = comb_form_var[[i]], vardir = sigma2d, method = "ML", B = 200, data = data_example) ## Fit the model
...
shapley(KIC, nvar) ### Obtain the Shapley Value


#### Compute the nested models according to Shapley Value importance

modelX2 <- eblupFH(yd ~ 0 + X1 , vardir = sigma2d, data = data_example)
modelX2$fit$goodness
modelX2$fit$estcoef[,4]

modelX21 <- eblupFH(yd ~ 0 + X1 + X2, vardir = sigma2d, data = data_example)
modelX21$fit$goodness
modelX21$fit$estcoef[,4]

modelX213 <- eblupFH(yd ~ 0 + X1 + X2 + X3, vardir = sigma2d, data = data_example)
